...
const char * bc_executable_name = "translate";
const char * bc_executable_description = "translates the geometry described by a bc stream";
...
static bc_number_t bc__translation[3];
...
void bc_executable_cli(void) {
  bc_cli_number("x", "x", "translation on the x axis (millimeters)", &bc__translation[0], 0.0f);
  bc_cli_number("y", "y", "translation on the y axis (millimeters)", &bc__translation[1], 0.0f);
  bc_cli_number("z", "z", "translation on the z axis (millimeters)", &bc__translation[2], 0.0f);
}
...
void bc_executable_nullary(
  ...
  if (opcode >= BC_OPCODE_PARAMETER(0) && opcode <= BC_OPCODE_PARAMETER(2)) {
    ...
    if (!bc__parameters_generated[parameter]) {
      ...
      if (bc__translation[parameter]) {
        result = bc_write_bc_binary(
          BC_OPCODE_SUBTRACT,
          original_parameter,
          bc_argument_number_constant(bc__translation[parameter])
        );
      } else {
