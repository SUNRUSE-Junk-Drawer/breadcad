...
void sdf_executable_cli(void) {
  sdf_cli_float("sxy", "size-xy", "size on x and y axes (diameter, millimeters)", &sdf__size_xy, 1.0f);
  sdf_cli_float("sz", "size-z", "size on z axis (length/height, millimeters)", &sdf__size_z, 1.0f);
...
static sdf_argument_t sdf__xy(
  ...
  if (sdf__center_xy) {
    ...
    return sdf_write_sdf_binary(SDF_OPCODE_SUBTRACT, position, sdf_argument_float_constant(sdf__size_xy / 2.0f));
  }
}
...
static sdf_argument_t sdf__distance_xy(void) {
  ...
  return sdf_write_sdf_binary(SDF_OPCODE_SUBTRACT, length, sdf_argument_float_constant(sdf__size_xy / 2.0f));
}
...
static sdf_argument_t sdf__distance_z(void) {
  ...
  if (sdf__center_z) {
    negative_distance = sdf_write_sdf_binary(SDF_OPCODE_SUBTRACT, sdf_argument_float_constant(sdf__size_z / -2.0f), position);
    positive_distance = sdf_write_sdf_binary(SDF_OPCODE_SUBTRACT, position, sdf_argument_float_constant(sdf__size_z / 2.0f));
  } else {
    negative_distance = sdf_write_sdf_unary(SDF_OPCODE_NEGATE, position);
    positive_distance = sdf_write_sdf_binary(SDF_OPCODE_SUBTRACT, position, sdf_argument_float_constant(sdf__size_z));
  }
...
void sdf_executable_before_first_file(void) {
  ...
  sdf_argument_t distance_xy_positive = sdf_write_sdf_binary(SDF_OPCODE_MAX, distance_xy, sdf_argument_float_constant(0.0f));
  ...
  sdf_argument_t distance_z_positive = sdf_write_sdf_binary(SDF_OPCODE_MAX, distance_z, sdf_argument_float_constant(0.0f));
  ...
  sdf_argument_t negative_distance = sdf_write_sdf_binary(SDF_OPCODE_MIN, greatest_distance, sdf_argument_float_constant(0.0f));
